 ...
 * SJRNSched.java
 * 
 * Shortest Job Remaining Next: Position in queue is ordered on which 
 * jobs need the least amount of CPU/IO time to complete 
 * 
...
public class SJRNSched{
	
    protected static List<Process> processList;
	protected static ArrayList<Process> arrivalList;  
	
	protected static ArrayList<Process> queue;
	
	static PriorityQueue<Process> cpuQueue, IOQueue; 
	...
	public SJRNSched(List<Process> processes, int numProc, int de)
	{
...
//------- add to CPU array---------		
			  ...
			  currProc = cpuQueue.peek();  //get next task on queue 
			...
			
			{
				...
				//System.out.println("Proc " + currProc.getID() + " has " + currProc.procTimeRmg + "time remaining on current process");
				...
				currProc.procTimeRmg --; //deceases procTime remaining
...
//-----------readjust queues after cpuProc Cnts change-------
			...
			if ((currProc!= null) && (currProc.procTimeCnt == currProc.getProcTime()))  //burst finished
			   {	
				 ...
				 currProc.procTimeRmg = currProc.getProcTime(); //reset time required for burst	
				 ...
				 if ((currProc.burstCnt)==(currProc.getNumBursts()))  // all bursts complete
					{     
					  ...
					  cpuQueue.poll(); //remove from head
...
//------- readjust queues after IOProc Cnts change---------	
	...
	{	
      ...
      System.out.print("(unordered) cpuQueue: [" );
	...
	{	
		cpuQueue =  new PriorityQueue<Process>(numProc, new Comparator<Process>()
		{ 	 public int compare(Process p1, Process p2) 
			{	return p1.procTimeRmg.compareTo(p2.procTimeRmg);	}
		}
		);
		IOQueue =  new PriorityQueue<Process>(numProc, new Comparator<Process>()
		{  public int compare(Process p1, Process p2) 
			{	return p1.currArrivTime.compareTo(p2.currArrivTime);	}
		});	
	}		
